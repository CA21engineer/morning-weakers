...
class HackathonRepository {
  ...
  Future<Hackathon> getHackathon(String hackathonId) async {
    final DocumentReference ref = firestore.collection('hackathons/v1').document(hackathonId);
  ...
  Future<Joined> getMyHackathons() async {
    // TODO: userIdはAuthができたら変更
    final DocumentReference joinedRef = firestore.collection('joined/v1').document('userId');
    return joinedRef.get().then((value) => Joined.fromJson(value.data));
  }
}

// TODO: authができたらUserはauthから取得する
Future<void> joinHackathon(Hackathon hackathon,
